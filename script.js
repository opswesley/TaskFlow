document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTasks(filter = activeFilter) {
        ...
        const filteredTasks = tasks.filter(task => {
            if (filter === 'completed') return task.completed && !task.expired; // Concluídas e não expiradas
            if (filter === 'pending') return !task.completed && !task.expired; // Pendentes e não expiradas
            if (filter === 'expired') return task.expired; // Expiradas
            return true; // Todas as tarefas
        });
    ...
    function checkExpiredTasks() {
        ...
        renderTasks(activeFilter); // Respeitar o filtro ativo
    }
    ...
    filterButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            activeFilter = button.dataset.filter; // Atualizar o filtro ativo
